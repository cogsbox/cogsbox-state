...
export function PixelRain({ numberOfDrops = 200 }: PixelRainProps) {
  ...
  const BASE_SKYLINE_BRIGHTNESS = 0.7;
  const [skylineBrightness, setSkylineBrightness] = useState(
    BASE_SKYLINE_BRIGHTNESS
  );
    ...
    const triggerLightning = () => {
      ...
      fadeInterval = setInterval(() => {
        ...
        if (currentIntensity < 0.01) {
          ...
          setSkylineBrightness(BASE_SKYLINE_BRIGHTNESS);
          clearInterval(fadeInterval);
          fadeInterval = undefined;
        } else {
          // --- THIS IS THE FIX ---
          // Keep the gradient element visible throughout the entire flicker.
          setLightningFlash(true);

          if (isLit) {
            // "ON" STATE: Set to full brightness for this frame.
            setLightningIntensity(currentIntensity);
            const newBrightness =
              BASE_SKYLINE_BRIGHTNESS - currentIntensity * 0.5;
            setSkylineBrightness(Math.max(0.2, newBrightness));
          } else {
            // "OFF" STATE IS NOW A "DIM" STATE:
            // Instead of turning off, set intensity to a fraction of the current brightness.
            const dimIntensity = currentIntensity * 0.3; // e.g., 30% brightness
            setLightningIntensity(dimIntensity);

            // Set the skyline brightness to a correspondingly dimmer state.
            const dimBrightness = BASE_SKYLINE_BRIGHTNESS - dimIntensity * 0.5;
            setSkylineBrightness(dimBrightness);
          }
        }

        // The flicker switch remains, pulsing between the "ON" and "DIM" states.
        isLit = !isLit;
      }, 40); // You can increase this to 50 or 60 to make the strobe slower
    };
  ...
  const steppedGradient = useMemo(() => {
    const steps = 20;
    const startColor = { r: 56, g: 70, b: 90 };
  ...
  return (
    <>
      ...
      />
      {/* 3. (FIX) Corrected the z-index to sit behind the skyline */}
      <div
        style={{
          position: "fixed",
          inset: 0,
      ...
      {skylineMaskUri && (
        <div
          style={{
            ...
            zIndex: -2,
            pointerEvents: "none",
            // 4. (FIX) Simplified the filter to use the calculated brightness
            filter: `brightness(${skylineBrightness})`,
